...
CREATE INDEX idx_convpayments_session ON conversation_payments (session_id);
...
-- ------------------------------------------------------------
-- USER REPORTS (students/tutors report other users)
-- ------------------------------------------------------------
...
-- ------------------------------------------------------------
-- USER WARNINGS (admin warnings issued to users)
-- ------------------------------------------------------------
...
CREATE INDEX idx_user_warnings_user ON user_warnings (user_id, is_acknowledged);
...
-- ------------------------------------------------------------
-- TUTOR APPLICATIONS (tutor registration flow; admin approves
-- before the tutor account is created)
-- ------------------------------------------------------------
...
CREATE INDEX idx_tutor_applications_created ON tutor_applications (created_at);
...
-- Conversation payments are tied to the exact session they paid for. This prevents
-- a cancelled booking's payment from "carrying over" onto a new booking in the same
-- conversation (which reused the old conversation_id).
